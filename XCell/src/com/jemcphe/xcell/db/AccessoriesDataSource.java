...
public class AccessoriesDataSource {
	...
	public void delete() {
		dataBase.delete(AccessoryDBOpenHelper.TABLE_ACCESSORIES, null, null);
	}
	...
	public ArrayList<Double> accessoryTotals(){
		...
		if(cursor.getCount() > 0){
			cursor.moveToFirst();
			while (!cursor.isAfterLast()){
				Accessory total = cursorToAccessory(cursor);
				totalString = total.getTotal();
				totalValue = Double.valueOf(totalString);
				totals.add(totalValue);
				cursor.moveToNext();

				Log.i(LOGTAG, totals.toString());
			}
		}
		cursor.close();
		return totals;
